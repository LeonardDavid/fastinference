...
float layer2_gpu_cuda(int * cuda_layer_1_output, int * cuda_layer_2_output){
    ...
    cudaMalloc((void **) &d_cuda_layer_1_output, 128*1*1*28*28*sizeof(int)); // dim of cuda_layer_1_output
    ...
    cudaMalloc((void **) &d_cuda_layer_2_output, 128*1*32*26*26*sizeof(int)); // dim of layer_2_output
    cudaCheckErrors("Failed to allocate device buffer");

    // copy input data from host on device
    cudaMemcpy(d_cuda_layer_1_output, cuda_layer_1_output, (128*1*1*28*28*sizeof(int)), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(cuda_layer_2_output, d_cuda_layer_2_output, (128*1*32*26*26*sizeof(int)), cudaMemcpyDeviceToHost);
    cudaCheckErrors("CUDA memcpy failure");
...
float layer4_gpu_cuda(unsigned int * cuda_layer_3_output, unsigned int * cuda_layer_4_output){
    ...
    // allocate GPU device buffers
    // Note: batch_size included in input and output shapes
    cudaMalloc((void **) &d_cuda_layer_3_output, 128*1*32*26*26*sizeof(unsigned int)); // dim of cuda_layer_3_output
    cudaMalloc((void **) &d_cuda_layer_4_output, 128*1*32*13*13*sizeof(unsigned int)); // dim of layer_4_output
    cudaCheckErrors("Failed to allocate device buffer");

    // copy input data from host on device
    cudaMemcpy(d_cuda_layer_3_output, cuda_layer_3_output, (128*1*32*26*26*sizeof(unsigned int)), cudaMemcpyHostToDevice);
    ...
    // copy result from device to host
    cudaMemcpy(cuda_layer_4_output, d_cuda_layer_4_output, (128*1*32*13*13*sizeof(unsigned int)), cudaMemcpyDeviceToHost);
    cudaCheckErrors("CUDA memcpy failure");
...
float layer7_gpu_cuda(unsigned int * cuda_layer_6_output, unsigned int * cuda_layer_7_output){
    ...
    // allocate GPU device buffers
    // Note: batch_size included in input and output shapes
    cudaMalloc((void **) &d_cuda_layer_6_output, 128*1*32*11*11*sizeof(unsigned int)); // dim of cuda_layer_6_output
    cudaMalloc((void **) &d_cuda_layer_7_output, 128*1*32*5*5*sizeof(unsigned int)); // dim of layer_7_output
    cudaCheckErrors("Failed to allocate device buffer");

    // copy input data from host on device
    cudaMemcpy(d_cuda_layer_6_output, cuda_layer_6_output, (128*1*32*11*11*sizeof(unsigned int)), cudaMemcpyHostToDevice);
    ...
    // copy result from device to host
    cudaMemcpy(cuda_layer_7_output, d_cuda_layer_7_output, (128*1*32*5*5*sizeof(unsigned int)), cudaMemcpyDeviceToHost);
    cudaCheckErrors("CUDA memcpy failure");
...
float layer9_gpu_cuda(unsigned int * cuda_layer_8_output, signed int * cuda_layer_9_output){
    ...
    // allocate GPU device buffers
    // Note: batch_size included in input and output shapes
    cudaMalloc((void **) &d_cuda_layer_8_output, 128*1*800*sizeof(unsigned int)); // dim of cuda_layer_8_output
    ...
    cudaMalloc((void **) &d_cuda_layer_9_output, 128*1*32*sizeof(signed int)); // dim of layer_9_output
    cudaCheckErrors("Failed to allocate device buffer");

    // copy input data from host on device
    cudaMemcpy(d_cuda_layer_8_output, cuda_layer_8_output, (128*1*800*sizeof(unsigned int)), cudaMemcpyHostToDevice);
    ...
    */
    ...
    // copy result from device to host
    cudaMemcpy(cuda_layer_9_output, d_cuda_layer_9_output, (128*1*32*sizeof(signed int)), cudaMemcpyDeviceToHost);
    cudaCheckErrors("CUDA memcpy failure");
...
float layer11_gpu_cuda(unsigned int * cuda_layer_10_output, signed int * cuda_layer_11_output){
    ...
    // allocate GPU device buffers
    // Note: batch_size included in input and output shapes
    cudaMalloc((void **) &d_cuda_layer_10_output, 128*1*32*sizeof(unsigned int)); // dim of cuda_layer_10_output
    ...
    cudaMalloc((void **) &d_cuda_layer_11_output, 128*1*10*sizeof(signed int)); // dim of layer_11_output
    cudaCheckErrors("Failed to allocate device buffer");

    // copy input data from host on device
    cudaMemcpy(d_cuda_layer_10_output, cuda_layer_10_output, (128*1*32*sizeof(unsigned int)), cudaMemcpyHostToDevice);
    ...
    */
    ...
    // copy result from device to host
    cudaMemcpy(cuda_layer_11_output, d_cuda_layer_11_output, (128*1*10*sizeof(signed int)), cudaMemcpyDeviceToHost);
    cudaCheckErrors("CUDA memcpy failure");
