...
def test_implementations(model, dataset, split, implementations, now, base_optimizers = [([None], [{}])], out_path = ".", model_name="Model", impl_folder = "cuda", n_repeat=1):
    ...
    b_l = 3
    b_u = 4
    # max b_u = 9 => max 2^8 = 256 (batch_size)
    ...
    for entry in range(len(performance)):
        ...
        time_all += performance[entry].get("total_time [s]")
    ...
    with open(profile_path + "/optimal.txt", 'a') as f:
        print("optimal times for each batch size:")
        f.write("optimal times for each batch size:\n")
        for time in range(len(optimal_times)):
            print("batch_size %d: %.2f + %.2f = %.2f" % (optimal_times[time][0], optimal_times[time][1], optimal_times[time][2], optimal_times[time][3]))
            f.write("batch_size %d: %.2f + %.2f = %.2f\n" % (optimal_times[time][0], optimal_times[time][1], optimal_times[time][2], optimal_times[time][3]))
        print("\n")
        f.write("\n")
        ...
        f.write("optimal batch_size: %d, cpu_time: %.2f, gpu_time: %.2f, total_time: %.2f\n" % (optimal_batch_size, optimal_times[0][1], optimal_times[0][2], optimal_times[0][3]))
        print("\n")
        f.write("\n")
        ...
        f.write("optimal configuration for layer:\n")
        for layer in range(len(optimal_config[optimal_batch_size])):
            ...
            f.write("%d: implem: %s\n" % (layer+1, optimal_config[optimal_batch_size][layer][2]))
