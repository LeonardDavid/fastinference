...
def to_implementation(model, out_path, out_name, weight = 1.0, namespace = "FAST_INFERENCE", align = 0, feature_type = "int", label_type = "int", float_type = "double", int_type = "signed int", uint_type = "unsigned int", infer_types = True, popcount = None, batch_size = 1, **kwargs):
    ...
    Otherwise the supplied data types are used.  
    Note that the first layer is never binarized because we do not assume the input data to be in a packed integer format, but to be a regular array. 
    ...
    **Important**: This implementation generates gcc compliant code by using the :code:`__builtin_popcount` or :code:`__builtin_popcountll` (depending on the uint_type) intrinsic. 
    This intrinsic can vary from compiler to compiler. 
    If you want to compile with another compiler (e.g. MSVC or clang) then you can supply the corresponding popcount operation via the popcount argument. 
    However, please keep in mind that you might have to adapt the included headers manually after the code generation and that the popcount operation should fit the corresponding uint_type.
    ...
    Args:
        ...
        batch_size (signed int): size of batch
    ...
    opt_implem = kwargs.get("opt_impl")
    # print(opt_implem)
    print("\n")
    ...
    for layer_id, layer in enumerate(model.layers):
        print("IMPLEMENTING {}: {} ({})".format(layer_id+1, layer.name, opt_implem[layer_id]))
    ...
    pkg_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), opt_implem[0])
    # base, header, etc. templates are the same for every profile => doesn't matter which opt_implem element is used
    env = Environment(
        loader=FileSystemLoader(pkg_path),
        trim_blocks=True, lstrip_blocks=True, keep_trailing_newline=True
    )
