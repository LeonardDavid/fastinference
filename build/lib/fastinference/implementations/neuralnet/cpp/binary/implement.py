...
def render(layer, input_type, layer_id = 0, is_first = False, float_type = "double", int_type = "int", uint_type = "unsigned int", infer_types = False, align = 0,popcount = None):
    ...
    if is_first and isinstance(layer, (Gemm, Conv2D)):
        ...
        if isinstance(layer, (LeakyRelu, Relu, Sigmoid, Sign, MaxPool2d, Reshape, BatchNorm)):
...
def to_implementation(model, out_path, out_name, weight = 1.0, namespace = "FAST_INFERENCE", align = 0, feature_type = "int", label_type = "int", float_type = "double", int_type = "signed int", uint_type = "unsigned int", infer_types = True, popcount = None, **kwargs):
    """
